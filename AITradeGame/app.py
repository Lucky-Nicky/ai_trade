...
# Initialize market fetcher with data source priority from settings
def get_market_fetcher():
    """Get market fetcher with current settings"""
    settings = db.get_settings()
    data_source_priority = settings.get('data_source_priority', 'odaily,528btc,binance,coingecko')
    priority_list = [s.strip() for s in data_source_priority.split(',')]
    return MarketDataFetcher(data_source_priority=priority_list)

market_fetcher = get_market_fetcher()
trading_engines = {}
auto_trading = True
...
def add_provider():
    ...
    try:
        ...
        if db.has_operation_password():
            password = data.get('password', '')
            if not db.verify_operation_password(password):
...
def test_api_connection():
    """Test API connection with specific provider and model"""
    data = request.json
    ...
    try:
        ...
        try:
            # Use the same base_url handling logic as ai_trader.py
            ...
            # Simple test message to verify the API works
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": "Hello, this is a connection test. Please respond with 'Test successful'."
                    }
                ],
                max_tokens=50,
                temperature=0.1
...
def get_portfolio(model_id):
    ...
    account_value = db.get_account_value_history(model_id, limit=100, time_range=time_range)
...
def get_aggregated_portfolio():
    ...
    for model in models:
        ...
        if portfolio:
            ...
            total_portfolio['initial_capital'] += portfolio.get('initial_capital', model.get('initial_capital', 100000))
    ...
    # Get multi-model chart data with time range support
    chart_data = db.get_multi_model_chart_data(limit=100, time_range=time_range)
...
def get_models_chart_data():
    ...
    time_range = request.args.get('time_range', None)
    chart_data = db.get_multi_model_chart_data(limit=limit, time_range=time_range)
...
def get_settings():
    ...
    try:
        ...
        if 'operation_password' in settings:
            settings['has_password'] = settings['operation_password'] is not None
            del settings['operation_password']
...
def update_settings():
    ...
    try:
        ...
        data_source_priority = data.get('data_source_priority', 'odaily,528btc,binance,coingecko')

        success = db.update_settings(trading_frequency_minutes, trading_fee_rate, data_source_priority)

        if success:
            # Reinitialize market fetcher with new settings
            market_fetcher = get_market_fetcher()
            print(f"[INFO] Settings updated, data source priority: {data_source_priority}")
...
def verify_password():
    ...
    if db.verify_operation_password(password):
...
def set_password():
    ...
    if db.has_operation_password():
        if not db.verify_operation_password(old_password):
    ...
    if db.set_operation_password(new_password):
        return jsonify({'success': True, 'message': '密码设置成功'})
    else:
        return jsonify({'success': False, 'error': '密码设置失败'}), 500

@app.route('/api/password/has', methods=['GET'])
def has_password():
    """Check if operation password is set"""
    return jsonify({'has_password': db.has_operation_password()})
