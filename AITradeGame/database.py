...
class Database:
    ...
    def init_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS settings (
                ...
                operation_password TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')

        # Migrate existing settings table to add data_source_priority column if not exists
        try:
            cursor.execute("SELECT data_source_priority FROM settings LIMIT 1")
        except sqlite3.OperationalError:
            # Column doesn't exist, add it
            cursor.execute('''
                ALTER TABLE settings ADD COLUMN data_source_priority TEXT DEFAULT 'odaily,528btc,binance,coingecko'
            ''')
            print("[INFO] Added data_source_priority column to settings table")

        # Migrate existing settings table to add operation_password column if not exists
        try:
            cursor.execute("SELECT operation_password FROM settings LIMIT 1")
        except sqlite3.OperationalError:
            # Column doesn't exist, add it
            cursor.execute('''
                ALTER TABLE settings ADD COLUMN operation_password TEXT
            ''')
            print("[INFO] Added operation_password column to settings table")

        # Insert default settings if no settings exist
        cursor.execute('SELECT COUNT(*) FROM settings')
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                INSERT INTO settings (trading_frequency_minutes, trading_fee_rate, data_source_priority)
                VALUES (60, 0.001, 'odaily,528btc,binance,coingecko')
            ''')
        else:
            # Update existing settings to have default data_source_priority if null
            cursor.execute('''
                UPDATE settings
                SET data_source_priority = 'odaily,528btc,binance,coingecko'
                WHERE data_source_priority IS NULL OR data_source_priority = ''
            ''')
    ...
    def get_account_value_history(self, model_id: int, limit: int = 100, time_range: str = None) -> List[Dict]:
        """Get account value history with optional time range filter"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        if time_range:
            ...
            if time_filter:
                cursor.execute(f'''
                    ...
                    WHERE model_id = ? AND timestamp >= datetime('now', '{time_filter}')
                    ORDER BY timestamp DESC LIMIT ?
                ''', (model_id, limit))
            else:
                # 'all' or invalid time_range, get all records
                cursor.execute('''
    ...
    def _get_time_filter(self, time_range: str) -> str:
        """Convert time_range string to SQLite datetime modifier"""
        time_filters = {
            '30m': '-30 minutes',
            '1h': '-1 hour',
            '6h': '-6 hours',
    ...
    def get_multi_model_chart_data(self, limit: int = 100, time_range: str = None) -> List[Dict]:
        ...
        for model in models:
            ...
            # Get account value history for this model with time range support
            if time_range:
                time_filter = self._get_time_filter(time_range)
                if time_filter:
                    cursor.execute(f'''
    ...
    def get_settings(self) -> Dict:
        ...
        cursor.execute('''
            SELECT trading_frequency_minutes, trading_fee_rate, data_source_priority, operation_password
            FROM settings
            ORDER BY id DESC
            LIMIT 1
        ''')
        ...
        if row:
            return {
                ...
                'data_source_priority': row['data_source_priority'] if row['data_source_priority'] else 'odaily,528btc,binance,coingecko',
                'operation_password': row['operation_password']
            }
        else:
            ...
            return {
                ...
                'data_source_priority': 'odaily,528btc,binance,coingecko',
                'operation_password': None
            }

    def update_settings(self, trading_frequency_minutes: int, trading_fee_rate: float, data_source_priority: str = None) -> bool:
        ...
        try:
            if data_source_priority is not None:
                ...
                ''', (trading_frequency_minutes, trading_fee_rate, data_source_priority))
            else:
                cursor.execute('''
                    UPDATE settings
                    SET trading_frequency_minutes = ?,
    ...
    def set_operation_password(self, password: str) -> bool:
        ...
        try:
            ...
            print(f"Error setting operation password: {e}")
            conn.close()
            return False

    def verify_operation_password(self, password: str) -> bool:
        """Verify operation password"""
        import hashlib
        settings = self.get_settings()
        stored_hash = settings.get('operation_password')
    ...
    def has_operation_password(self) -> bool:
        """Check if operation password is set"""
        settings = self.get_settings()
        return settings.get('operation_password') is not None
